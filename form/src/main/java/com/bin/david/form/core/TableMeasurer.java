...
public class TableMeasurer<T>
{
    ...
    private int getTableHeight (TableData<T> tableData, TableConfig config)
    {
        ...
        if (config.isShowColumnTitle())
        {
            ...
            {
                titleHeight = config.getColumnDateTitleFormat().measureHeight(config);
            }
            else
            {
                titleHeight = tableData.getTitleDrawFormat()
                        .measureHeight(config) + 2 * config.getColumnTitleVerticalPadding();
            }
        }
        TableInfo tableInfo = tableData.getTableInfo();
    ...
    private int getTableWidth (TableData<T> tableData, TableConfig config)
    {
        ...
        {
            int yAxisWidth =
                    (int) paint.measureText(tableData.getYSequenceFormat().format(totalSize)
    ...
    private void measureRowHeight (
            TableConfig config, int[] lineHeightArray, Column column,
    ...
    public ColumnInfo getColumnInfo (
            TableData<T> tableData, Column column, ColumnInfo parent,
